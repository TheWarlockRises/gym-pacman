...
class path_finder:
            ...
            if thisLowestFNode != self.end and thisLowestFNode != False:
                ...
                for offset in self.neighborSet:
                    ...
                    if not thisNeighbor[0] < 0 and not thisNeighbor[
                                                           1] < 0 and not \
                            thisNeighbor[0] > self.size[
                                0] - 1 and not thisNeighbor[1] > self.size[
                        1] - 1 and not self.GetType(thisNeighbor) == 1:
                        cost = self.GetG(self.current) + 10

                        if self.IsInOpenList(
                                thisNeighbor) and cost < self.GetG(
                            thisNeighbor):
    ...
    def CalcF(self, row_col):
        ...
        self.map[unfoldIndex].f = self.map[unfoldIndex].g + self.map[
            unfoldIndex].h
    ...
    def draw(self, screen, tileIDImage):
        ...
        for row in range(0, self.size[0], 1):
            for col in range(0, self.size[1], 1):
                ...
                screen.blit(tileIDImage[thisTile],
                            (col * (TILE_WIDTH * 2), row * (TILE_WIDTH * 2)))
